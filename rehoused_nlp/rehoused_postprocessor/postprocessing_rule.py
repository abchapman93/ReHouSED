    ...
    def __call__(self, ent, i, debug=False):
        ...
        try:
            if self.kwargs:
                ...
                self.action(
                    ent, i, **self.kwargs#self.input_span_type, self.span_group_name, **self.kwargs
                )
            else:

                self.action(ent, i, #self.input_span_type, self.span_group_name, **self.kwargs
                            )
        except TypeError:
            _raise_action_error(
                self.action,
                (ent, i, self.kwargs,
                 #self.input_span_type, self.span_group_name
                 ),
            )
        return True
